(() => {
  ...
  function processTourDepartures(root = document.getElementById('tourScreen')) {
    ...
    root.querySelectorAll('.depart-card').forEach(card => {
      ...
      const button = [...card.querySelectorAll('button')].find(item => /присоединиться|забронировать|лист ожидания/i.test(item.textContent || ''));
  ...
  globalThis.MaxTourDepartureLiveV3 = {
    ...
    _test:{ vietnamTodayIso, parseDepartureLabel, isFullCard },
  };
})();
