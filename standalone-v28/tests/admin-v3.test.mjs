...
test('admin v3 keeps the approved navigation and operational views', () => {
  ...
  assert.match(admin, /function renderDashboard\(/);
  assert.match(admin, /function renderDepartures\(/);
  assert.match(admin, /function renderOrders\(/);
  assert.match(admin, /function renderCustomers\(/);
});
