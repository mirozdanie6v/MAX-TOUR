...
export async function createOrder(env: Env, sessionId: string, draft: BookingDraft, tour: Tour, quote: Quote, idempotencyKey: string) {
  ...
  const snapshot = JSON.stringify({ quote, tourPricing: tour.pricingRules, paymentChoice: draft.paymentChoice, children: draft.children });
  ...
  await env.DB.prepare('INSERT INTO analytics_events(session_id,event_type,source,tour_id,order_id,amount_minor,metadata_json,demo) VALUES (?,?,?,?,?,?,?,1)').bind(sessionId,'order_created',draft.source,tour.id,displayId,quote.totalMinor,'{}').run();
