...
export async function patchTour(env: Env, sessionId: string, tourId: string, input: unknown) {
  ...
  if (current.dataStatus === 'userCreatedDemo') {
    ...
    await recordAudit(env,sessionId,'admin','tour.update','tour',current.id,current,next);
    return next;
  }
  ...
  await recordAudit(env,sessionId,'admin','tour.update','tour',current.id,current,after);
  return after;
}
...
export async function addTour(env: Env, sessionId: string, input: unknown) {
  ...
  await recordAudit(env,sessionId,'admin','tour.create','tour',id,null,tour);
  return tour;
}
...
export async function setAvailability(env: Env, sessionId:string, tourId:string, input:unknown){
  ...
  await recordAudit(env,sessionId,'admin','tour.availability.update','tour_availability',`${tour.id}:${date}`,before,after,{tourId:tour.id});
  return after;
}
...
export async function setPromo(env:Env,sessionId:string,tourId:string,input:unknown){
  ...
  await recordAudit(env,sessionId,'admin','tour.promo.update','tour_promo',tour.id,before,after);
  return after;
}
...
export async function addDirection(env:Env,sessionId:string,input:any){
  ...
  await recordAudit(env,sessionId,'admin','direction.create','direction',id,null,after);
  return after;
}
