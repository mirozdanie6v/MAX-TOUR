...
export async function patchTour(env: Env, sessionId: string, tourId: string, input: unknown, actorId = 'demo') {
  ...
  if (current.dataStatus === 'userCreatedDemo') {
    ...
    await recordAudit(env,sessionId,'admin','tour.update','tour',current.id,current,next,{},actorId);
    return next;
  }
  ...
  const after = await getTourByIdOrSlug(env.DB,sessionId,current.id);
  await recordAudit(env,sessionId,'admin','tour.update','tour',current.id,current,after,{},actorId);
  return after;
}
...
export async function addTour(env: Env, sessionId: string, input: unknown, actorId = 'demo') {
  ...
  await recordAudit(env,sessionId,'admin','tour.create','tour',id,null,tour,{},actorId);
  return tour;
}
...
export async function setAvailability(env: Env, sessionId:string, tourId:string, input:unknown, actorId = 'demo'){
  ...
  await recordAudit(env,sessionId,'admin','tour.availability.update','tour_availability',`${tour.id}:${date}`,before,after,{tourId:tour.id},actorId);
  return after;
}
...
export async function setPromo(env:Env,sessionId:string,tourId:string,input:unknown, actorId = 'demo'){
  ...
  await recordAudit(env,sessionId,'admin','tour.promo.update','tour_promo',tour.id,before,after,{},actorId);
  return after;
}
...
export async function addDirection(env:Env,sessionId:string,input:any, actorId = 'demo'){
  ...
  const after = {id,name,dataStatus:'userCreatedDemo'};
  await recordAudit(env,sessionId,'admin','direction.create','direction',id,null,after,{},actorId);
  return after;
}
