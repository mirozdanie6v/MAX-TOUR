...
one("import { api } from './lib/api';", "import { api } from './lib/api';\nimport { OwnerPage } from './OwnerPage';", 'owner import')
one("type Role = 'tourist' | 'manager' | 'admin';", "type Role = 'tourist' | 'manager' | 'admin' | 'owner';", 'role union')
one("const LOGO_SRC = 'https://thb.tildacdn.one/tild3938-3763-4364-a165-666362383464/-/resize/600x/Max_Tour-3.jpg';", "const LOGO_SRC = 'https://static.tildacdn.one/tild3063-6230-4266-b063-313839663461/____1680_x_600_-4.jpg';", 'logo url')
one("function roleFromPath(pathname: string): Role {\n  if (pathname.startsWith('/manager') || pathname.startsWith('/channels')) return 'manager';", "function roleFromPath(pathname: string): Role {\n  if (pathname.startsWith('/owner')) return 'owner';\n  if (pathname.startsWith('/manager') || pathname.startsWith('/channels')) return 'manager';", 'role path')
one("    <Route path=\"/channels\" element={<BackofficeShell title=\"Каналы продаж\" role=\"manager\"><ChannelsPage /></BackofficeShell>} />", "    <Route path=\"/channels\" element={<BackofficeShell title=\"Каналы продаж\" role=\"manager\"><ChannelsPage /></BackofficeShell>} />\n    <Route path=\"/owner\" element={<BackofficeShell title=\"Панель владельца\" role=\"owner\"><OwnerPage /></BackofficeShell>} />", 'owner route')

s,n=re.subn(r"function Logo\(\) \{.*?\n\}", '''function Logo() {
...
if n!=1: raise SystemExit('logo function replacement failed')

one("const goRole = (next: Role) => { setOpen(false); navigate(next === 'manager' ? '/manager' : next === 'admin' ? '/admin' : '/'); };", "const goRole = (next: Role) => { setOpen(false); navigate(next === 'manager' ? '/manager' : next === 'admin' ? '/admin' : next === 'owner' ? '/owner' : '/'); };", 'goRole')
one("<button className={role === 'admin' ? 'active' : ''} onClick={() => goRole('admin')}><b>Администратор</b><span>Каталог, цены, аналитика</span></button></div>", "<button className={role === 'admin' ? 'active' : ''} onClick={() => goRole('admin')}><b>Администратор</b><span>Каталог, цены, расписание</span></button><button className={role === 'owner' ? 'active' : ''} onClick={() => goRole('owner')}><b>Владелец</b><span>Показатели и правила команды</span></button></div>", 'owner drawer')

s,n=re.subn(r"function BackofficeShell\(\{ title, role, children \}: \{ title: string; role: Role; children: React\.ReactNode \}\) \{.*?\n\}\n\nfunction useTours", '''function BackofficeShell({ title, role, children }: { title: string; role: Role; children: React.ReactNode }) {
...
function useTours''',s,count=1,flags=re.S)
if n!=1: raise SystemExit('BackofficeShell replacement failed')

s,n=re.subn(r"function ManagerOrderPage\(\) \{.*?\n\}\n\nfunction ChannelsPage", '''function ManagerOrderPage() {
  const { id = '' } = useParams();
...
anchor="      if (path === '/api/admin/tours' && request.method === 'GET') return finish(json({ items: await adminTours(env, session.id) }));"
insert="""      m = match(path, /^\\/api\\/manager\\/orders\\/([^/]+)\\/ops$/);
...
"""+anchor
if anchor not in w: raise SystemExit('index route anchor missing')
w=w.replace(anchor,insert,1)
p.write_text(w)

# Session seed/reset
p=root/'src/worker/db/session.ts'; q=p.read_text()
old="  const events: Array<[string,string,string|null,string|null,number|null]> = ["
new="""  await env.DB.prepare('INSERT OR IGNORE INTO demo_owner_settings(session_id,manager_sla_minutes,manager_notifications,owner_digest,sales_focus) VALUES (?,15,1,?,?)').bind(sessionId,'Ежедневно','Премиум экскурсии').run();
  const opsSeed = [
    ...
    ['MT-DEMO-1041','Менеджер 1','Amiana — отдельная зона трансфера','Проверить трансфер и дату рождения участников — DEMO']
  ];
...
if old not in q: raise SystemExit('session event anchor missing')
q=q.replace(old,new,1)
oldtables="const tables = ['demo_tour_overrides','demo_user_created_tours','demo_availability','demo_promotions','demo_directions','manager_status_history','payments','order_participants','orders','analytics_events'];"
newtables="const tables = ['demo_tour_overrides','demo_user_created_tours','demo_availability','demo_promotions','demo_directions','demo_order_operations','demo_owner_settings','manager_status_history','payments','order_participants','orders','analytics_events'];"
if oldtables not in q: raise SystemExit('reset tables anchor missing')
q=q.replace(oldtables,newtables,1)
p.write_text(q)

# CSS overrides and owner styles
p=root/'src/client/styles/premium.css'; c=p.read_text()
c += r'''

/* v2 role/owner refinements */
...
'''
p.write_text(c)
print('role logic v2 patch applied')
